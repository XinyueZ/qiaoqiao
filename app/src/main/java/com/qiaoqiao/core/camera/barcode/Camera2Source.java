...
public class Camera2Source {
	...
	private Size getBestAspectPictureSize(android.util.Size[] supportedPictureSizes) {
		float targetRatio = DeviceUtils.getScreenRatio(mContext);
		Size bestSize = null;
	...
	private void setUpCameraOutputs(int width, int height) {
		try {
			...
			Point displaySize = new Point(DeviceUtils.getScreenWidth(mContext), DeviceUtils.getScreenHeight(mContext));
			...
			Size[] outputSizes = DeviceUtils.sizeToSize(map.getOutputSizes(SurfaceTexture.class));
